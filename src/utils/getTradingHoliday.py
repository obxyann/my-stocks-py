...
# 和平紀念日 2/28
# 兒童節     4/4
# 勞動節     5/1   (NOTE: <2025 僅勞工放假但休市, since 2025 全國放假)
...
#
# 民俗節日
# 小年夜   農 12 月末日之前一日 (農12/28 或 12/29) (NOTE: since 2025)
# 除夕     農 12 月末日 (農12/29 或 12/30)
# 春節     農 1/1 ~ 1/3 (三日)
# 清明節   農 清明 (P.S. 國4/4或5或6）
# 端午節   農 5/5
# 中秋節   農 8/15
#
# NOTE: 1. 兒童節與清明節同一日時，於前一日放假。但逢星期四時，於後一日放假
#       2. 立法院在 2025/5 三讀通過《紀念日及節日實施條例》修正案的結果 (5/27 公告實施)
#
# 補假原則
# --------
#   注意 102/03/05 ~ 112/05/25 各次不同歷史修正 (民之所欲，得改...)
#   https://law.dgpa.gov.tw/LawContentHistoryList.aspx?id=GL000247
#
#   114/06/13 修正
#   1. 放假日逢星期六者於前一個上班日補假，逢星期日者於次一個上班日補假
#   2. 但農曆除夕及春節放假日逢例假日，得於前一個或次一個上班日補假
#   3. 除夕及春節連續假期之次一個上班日為星期五者，得調整和平紀念日之補假日至該日補假
#
# 逢雨大就放假
# ------------
# 台北市歷次天然災害停止上班上課訊息
...
# 非六日例假日與休市日列表
# ------------------------
# NOTE: 1. 2019 起周末(六/日)無論是否補班皆不開市
#          (原 2015 ~ 2018 周六補班獨步全球有開市，<2015 待確定)
#       2. 以下僅列非周末放假日，如 2022-01-01 是周六，本來就放假不列
trading_holiday_db = [
    # 民國 108 年
    ...
    '2026-02-18',  # (三) 春節2
    '2026-02-19',  # (四) 春節3
    '2026-02-20',  # (五) 補 0215小年夜(逢日)
    '2026-02-27',  # (五) 補 0228和平(逢六)
    '2026-04-03',  # (五) 補 0404兒童(逢六)
    '2026-04-06',  # (一) 補 0405清明(逢日)
    '2026-05-01',  # (五) 勞動
    '2026-06-19',  # (五) 端午
    '2026-09-25',  # (五) 中秋
    '2026-09-28',  # (一) 教師
    '2026-10-09',  # (五) 補 1010國慶(逢六)
    '2026-10-26',  # (一) 補 1025光復(逢日)
    '2026-12-25',  # (五) 行憲
]

# for checking by rule (wait to implement, see below NOTE)
# PLAN: by 紀念日及節日實施條例 + 補假原則
#          農曆陽曆轉換表
#          24節氣表
# NOTE: 補假/調假 -> 常變動, 想不到簡單方式計算
#       災害假 -> 無法計算
"""
1. check date
non_trad_holiday = [
...
# '12-29' or '12-28' or '01-01'-2d, # 小年夜 (NOTE: since 2025)
]
...
def isTradingHoliday(the_date=None):
    ...
    if d.year < min_year or d.year > max_year:
        reason = f'Checking holiday only from {min_year}-01-01 to {max_year}-12-31'
