...
def get_last_market_close_day(
    close_hour=15, close_minute=0, min_guo_year=False, separator=None
):
    # current time
    ...
    market_close = curr_time.replace(
...
def file_is_old(path_name, hour=0, minute=0, second=0, quiet=True):
    ...
    if curr_time > last_time:
        quiet or print('    Update data is availabe')
            ...
            try:
                with zipfile.ZipFile(
                    ...
                    if dst_path in zipf.namelist():
                        print(f"    {EXCLAMATION_MARK} Duplicate name '{dst_path}'")  # HEAVY BALLOT X, U+2718 # fmt: skip
                        return 'skipped'
                    else:
